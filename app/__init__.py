    ...
    if conn.entries:
        ...
        if conn.bind():
            ...
            conn.search(entry.entry_dn,
                        '(objectclass=*)',
                        attributes=['mail', 'displayName', 'description', 'givenName', 'userPrincipalName', 'memberOf']
                    )
            if conn.entries:                    
